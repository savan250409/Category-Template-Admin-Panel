...
@section('container')
    <style>
        ...
        .data-table .col-file { width: 35%; }
        .data-table .col-premium { width: 12%; }
        ...
        .font-preview-thumb { width: 64px; height: 64px; object-fit: contain; padding: 4px; border-radius: .35rem; border: 1px solid #e3e6f0; background:#f8f9fc; }
        .file-link { color: #0d6efd; text-decoration: none; word-break: break-all; }
    ...
    <div class="container mt-4 mb-5">
        <div class="page-hero">
            <div class="hero-inner">
                <div class="hero-left">
                    <span class="hero-icon"><i class="bi bi-fonts"></i></span>
                    <div>
                        <h1 class="hero-title">Font Management</h1>
                        <p class="hero-subtitle">Upload and manage fonts available in your app</p>
                    </div>
                </div>
                <div class="hero-actions">
                    <span class="hero-count"><i class="bi bi-collection"></i> Total <span class="badge-num" id="totalCount">{{ $fonts->total() }}</span></span>
                    <a href="{{ route('fonts.create') }}" class="hero-btn hero-btn-add">
                        <i class="bi bi-plus-lg"></i> Add Font
                    </a>
                </div>
            </div>
        </div>
            ...
            <div class="filters-row">
                ...
                <div class="search-container">
                    <div class="input-group">
                        <input type="text" id="searchInput" class="form-control" placeholder="Search fonts..." value="{{ $search }}">
        ...
        $(document).ready(function () {
            ...
            function loadFonts(page) {
                ...
                $.ajax({
                    ...
                    error: function (xhr) {
                        Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to load fonts (' + xhr.status + ').' });
                    },
                    complete: function () {
            ...
            $('#per_page').on('change', function () { loadFonts(1); });

            let searchTimer = null;
            $('#searchInput').on('keyup', function (e) {
                clearTimeout(searchTimer);
                if (e.key === 'Enter') { loadFonts(1); return; }
                searchTimer = setTimeout(() => loadFonts(1), 500);
            });
