...
@section('container')
    <style>
        ...
        .data-table .col-values { width: 30%; }
        .data-table .col-type { width: 14%; }
        .data-table .col-action { width: 18%; }
        .values-cell { color:#5a5c69; font-size:.86rem; line-height:1.35; font-family: 'Courier New', monospace; }
        .action-btn { display: inline-flex; align-items: center; justify-content: center; width: 30px; height: 30px; border-radius: .35rem; color: #fff !important; text-decoration: none; border: none; }
        .edit-btn { background-color: #0dcaf0; }
        .delete-btn { background-color: #bb2d3b; }
        .action-cell { display: flex; justify-content: flex-end; gap: .5rem; }
        ...
        .per-page-select, .cat-filter-select { border: 1px solid #d1d3e2; border-radius: .35rem; padding: .5rem .75rem; background-color: #fff; }
        .per-page-select { width: 88px; }
        .cat-filter-select { min-width: 180px; }
        .search-container .input-group { width: 320px; }
        ...
        .pagination-info { white-space: nowrap; flex-shrink: 0; }
        ...
        .loading-overlay { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(255, 255, 255, .7); display: flex; justify-content: center; align-items: center; z-index: 10; }
        ...
        .check-cell .check-label { font-size: .85rem; font-weight: 600; color: #4a4a4a; }
        ...
        .hero-icon { width: 56px; height: 56px; border-radius: 14px; display: inline-flex; align-items: center; justify-content: center; background: rgba(255,255,255,.22); color: #fff; font-size: 1.6rem; }
        ...
        .hero-btn-import { background: #1976d2; color: #fff; }
        .hero-btn-import:hover { color: #fff; filter: brightness(1.05); }
    ...
    <div class="container mt-4 mb-5">
        <div class="page-hero">
            <div class="hero-inner">
                <div class="hero-left">
                    <span class="hero-icon"><i class="bi bi-sliders"></i></span>
                    <div>
                        <h1 class="hero-title">Filter Management</h1>
                        <p class="hero-subtitle">Manage filters</p>
                    </div>
                </div>
                <div class="hero-actions">
                    <span class="hero-count"><i class="bi bi-collection"></i> Total <span class="badge-num" id="totalCount">{{ $filters->total() }}</span></span>
                    <a href="{{ route('filter.filters.importForm') }}" class="hero-btn hero-btn-import">
                        <i class="bi bi-upload"></i> Import CSV
                    </a>
                    <a href="{{ route('filter.filters.create') }}" class="hero-btn hero-btn-add">
        ...
        <div class="main-card">
            <div class="filters-row">
                <div class="filters-left">
                    ...
                    <select id="category_filter" class="cat-filter-select">
                ...
                <div class="search-container">
                    <div class="input-group">
                        <input type="text" id="searchInput" class="form-control" placeholder="Search filters..." value="{{ $search }}">
    ...
    <script>
        $(document).ready(function () {
            ...
            function load(page) {
                ...
                $.ajax({
                    url: "{{ route('filter.filters.index') }}",
                    type: 'GET',
                    data: {
                        page: page || 1,
            ...
            $('#per_page, #category_filter').on('change', function () { load(1); });
            ...
            $(document).on('change', '.toggle-premium', function () {
                ...
                $.ajax({
                    url: "{{ route('filter.filters.togglePremium') }}",
                    method: 'POST',
