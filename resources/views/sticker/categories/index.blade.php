...
@section('title', 'Sticker Category Management')
@section('container')
    <style>
        ...
        .data-table .col-name { width: 35%; }
        .data-table .col-thumb { width: 17%; }
        .data-table .col-premium { width: 14%; }
        .data-table .col-status { width: 14%; }
        ...
        .cat-thumb { width: 56px; height: 56px; object-fit: cover; border-radius: .35rem; }
        ...
        .hero-btn-reorder:hover:not(:disabled) {
            color: #fff; transform: translateY(-2px); filter: brightness(1.05);
            box-shadow: 0 12px 24px rgba(255, 122, 89, .45), inset 0 0 0 1px rgba(255,255,255,.22);
        }

        /* Add Category — bright white pill with animated glow */
        .hero-btn-add {
        ...
        .hero-btn-add:hover {
            color: #5f37d9; transform: translateY(-2px);
            box-shadow: 0 14px 26px rgba(0,0,0,.22), 0 0 0 6px rgba(255,255,255,.18);
            animation: none;
        }
        ...
        .check-cell .form-check-input:checked {
    ...
    <div class="container mt-4 mb-5">
        <div class="page-hero">
            <div class="hero-inner">
                <div class="hero-left">
                    <span class="hero-icon"><i class="bi bi-emoji-smile-fill"></i></span>
                    <div>
                        <h1 class="hero-title">Sticker Category Management</h1>
                        <p class="hero-subtitle">Create, organize and curate the sticker categories shown in your app</p>
                    </div>
                </div>
                <div class="hero-actions">
                    <span class="hero-count"><i class="bi bi-collection"></i> Total <span class="badge-num" id="totalCount">{{ $categories->total() }}</span></span>
                    <button type="button" class="hero-btn hero-btn-reorder" data-bs-toggle="modal" data-bs-target="#indexingModal" @if($categories->total() === 0) disabled title="No categories to reorder" @endif>
                        <i class="bi bi-arrow-down-up"></i> Reorder
                    </button>
                    <a href="{{ route('sticker.categories.create') }}" class="hero-btn hero-btn-add">
        ...
        <div class="main-card">
            ...
            <div id="ajax-container">
                @include('sticker.categories.table')
            </div>
        </div>
    </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="indexingModalLabel">
                        <i class="bi bi-arrow-up-down me-2"></i>Reorder Sticker Categories
                    </h5>
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        ...
                        Drag and drop to reorder the categories. The sequence here controls the API response order.
                    </div>
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.delete-btn', function () {
                ...
                Swal.fire({
                    ...
                    text: 'This will delete "' + name + '" and all its stickers and files.',
                    icon: 'warning',
            ...
            $('#searchInput').on('keyup', function (e) {
                ...
                if (e.key === 'Enter') { loadCategories(1); return; }
            ...
            // Toggle status / premium (delegated)
            $(document).on('change', '.toggle-status', function () {
                const id = $(this).data('id');
                const status = $(this).is(':checked') ? 1 : 0;
                $.ajax({
                    url: "{{ route('sticker.categories.toggleStatus') }}",
                    method: 'POST',
            ...
            $(document).on('change', '.toggle-premium', function () {
                ...
                $.ajax({
                    url: "{{ route('sticker.categories.togglePremium') }}",
                    method: 'POST',
            ...
            if (indexingModalEl) {
                indexingModalEl.addEventListener('show.bs.modal', loadCategoriesForIndexing);
            }
            ...
            function loadCategoriesForIndexing() {
                ...
                $.ajax({
                    url: "{{ route('sticker.categories.indexing') }}",
                    type: 'GET',
                    success: function (response) {
                        if (response.categories && response.categories.length > 0) {
                            container.innerHTML = '';
                            response.categories.forEach((c, index) => {
                                const thumb = c.image_url
                                    ? '<img src="' + c.image_url + '" style="width:40px;height:40px;object-fit:cover;border-radius:.25rem;" class="me-2" alt="">'
                                    : '<span class="me-2 text-muted"><i class="bi bi-image"></i></span>';
                                const html = `
                                    <div class="list-group-item d-flex align-items-center justify-content-between sortable-item" data-id="${c.id}" style="cursor: move;">
                                        <div class="d-flex align-items-center">
                                            <i class="bi bi-grip-vertical me-2 text-muted fs-5"></i>
                                            <span class="fw-bold me-2">${index + 1}.</span>
                                            ${thumb}
                                            <span><strong>${$('<div>').text(c.category_name || '').html()}</strong></span>
                                        </div>
                                    </div>`;
                    ...
                    error: function () {
                        container.innerHTML = '<div class="col-12 text-center text-danger py-5"><p>Failed to load categories.</p></div>';
                    }
                });
            }
            ...
            document.getElementById('saveOrderBtn').addEventListener('click', function () {
                ...
                $.ajax({
                    url: "{{ route('sticker.categories.updateOrder') }}",
                    method: 'POST',
